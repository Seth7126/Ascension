    ...
    public final void onActivityDestroyed(Activity activity0) {
        if(activity0 == this.zzhm) {
            if(Log.isLoggable("NearbyMessagesClient", 2)) {
                Log.v("NearbyMessagesClient", "Unregistering ClientLifecycleSafetyNet\'s ActivityLifecycleCallbacks for com.playdekgames.android.Ascension");
            }
    ...
    public final void onActivityStopped(Activity activity0) {
        if(activity0 == this.zzhm) {
            try {
                ...
                if(Log.isLoggable("NearbyMessagesClient", 2)) {
                    Log.v("NearbyMessagesClient", String.format("Failed to emit ACTIVITY_STOPPED from ClientLifecycleSafetyNet for Activity %s: %s", "com.playdekgames.android.Ascension", remoteException0));
                }
            }
        }
    }
