...
final class RestrictedComponentContainer extends AbstractComponentContainer {
    static class RestrictedPublisher implements Publisher {
        ...
        public void publish(Event event0) {
            if(!this.allowedPublishedEvents.contains(event0.getType())) {
                throw new IllegalArgumentException(String.format("Attempting to publish an undeclared event %s.", event0));
            }
    ...
    public Object get(Class class0) {
        if(!this.allowedDirectInterfaces.contains(class0)) {
            throw new IllegalArgumentException(String.format("Attempting to request an undeclared dependency %s.", class0));
        }
    ...
    public Provider getProvider(Class class0) {
        if(!this.allowedProviderInterfaces.contains(class0)) {
            throw new IllegalArgumentException(String.format("Attempting to request an undeclared dependency Provider<%s>.", class0));
        }
    ...
    public Set setOf(Class class0) {
        if(!this.allowedSetDirectInterfaces.contains(class0)) {
            throw new IllegalArgumentException(String.format("Attempting to request an undeclared dependency Set<%s>.", class0));
        }
    ...
    public Provider setOfProvider(Class class0) {
        if(!this.allowedSetProviderInterfaces.contains(class0)) {
            throw new IllegalArgumentException(String.format("Attempting to request an undeclared dependency Provider<Set<%s>>.", class0));
        }
