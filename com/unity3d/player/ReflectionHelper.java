package com.unity3d.player;

import java.lang.invoke.MethodHandles.Lookup;
import java.lang.reflect.Array;
import java.lang.reflect.Constructor;
import java.lang.reflect.Field;
import java.lang.reflect.InvocationHandler;
import java.lang.reflect.Member;
import java.lang.reflect.Method;
import java.lang.reflect.Modifier;
import java.lang.reflect.Proxy;
import java.util.ArrayList;

final class ReflectionHelper {
    static final class a {
        public volatile Member a;
        private final Class b;
        private final String c;
        private final String d;
        private final int e;

        a(Class class0, String s, String s1) {
            this.b = class0;
            this.c = s;
            this.d = s1;
            this.e = ((class0.hashCode() + 0x20F) * 0x1F + s.hashCode()) * 0x1F + s1.hashCode();
        }

        // 去混淆评级： 中等(50)
        @Override
        public final boolean equals(Object object0) {
            return object0 == this ? true : object0 instanceof a && this.e == ((a)object0).e && this.d.equals(((a)object0).d) && this.c.equals(((a)object0).c) && this.b.equals(((a)object0).b);
        }

        @Override
        public final int hashCode() {
            return this.e;
        }
    }

    static final class b implements Runnable {
        final long a;
        final long b;

        public b(long v, long v1) {
            this.a = v;
            this.b = v1;
        }

        @Override
        public final void run() {
            if(!ReflectionHelper.beginProxyCall(this.a)) {
                return;
            }
            try {
                ReflectionHelper.nativeProxyFinalize(this.b);
            }
            finally {
                ReflectionHelper.endProxyCall();
            }
        }
    }

    public interface c extends InvocationHandler {
        void a(long arg1, boolean arg2);
    }

    protected static boolean LOG;
    protected static final boolean LOGV;
    private static a[] a;
    private static long b;
    private static long c;
    private static boolean d;

    static {
        ReflectionHelper.a = new a[0x1000];
    }

    private static float a(Class class0, Class class1) {
        if(class0.equals(class1)) {
            return 1.0f;
        }
        if(!class0.isPrimitive() && !class1.isPrimitive()) {
            try {
                if(class0.asSubclass(class1) != null) {
                    return 0.5f;
                }
            }
            catch(ClassCastException unused_ex) {
            }
            try {
                if(class1.asSubclass(class0) != null) {
                    return 0.1f;
                }
            }
            catch(ClassCastException unused_ex) {
            }
        }
        return 0.0f;
    }

    private static float a(Class class0, Class[] arr_class, Class[] arr_class1) {
        float f = 1.0f;
        int v = 0;
        if(arr_class1.length == 0) {
            return 0.1f;
        }
        if((arr_class == null ? 0 : arr_class.length) + 1 != arr_class1.length) {
            return 0.0f;
        }
        if(arr_class != null) {
            float f1 = 1.0f;
            for(int v1 = 0; v < arr_class.length; ++v1) {
                f1 *= ReflectionHelper.a(arr_class[v], arr_class1[v1]);
                ++v;
            }
            f = f1;
        }
        return f * ReflectionHelper.a(class0, arr_class1[arr_class1.length - 1]);
    }

    static long a() [...] // 潜在的解密器

    private static Class a(String s, int[] arr_v) {
    label_0:
        if(arr_v[0] < s.length()) {
            int v = arr_v[0];
            arr_v[0] = v + 1;
            int v1 = s.charAt(v);
            switch(v1) {
                case 40: 
                case 41: {
                    goto label_0;
                }
                case 66: {
                    return Byte.TYPE;
                }
                case 67: {
                    return Character.TYPE;
                }
                case 68: {
                    return Double.TYPE;
                }
                case 70: {
                    return Float.TYPE;
                }
                case 73: {
                    return Integer.TYPE;
                }
                case 74: {
                    return Long.TYPE;
                }
                case 76: {
                    int v2 = s.indexOf(59, arr_v[0]);
                    if(v2 != -1) {
                        String s1 = s.substring(arr_v[0], v2);
                        arr_v[0] = v2 + 1;
                        String s2 = s1.replace('/', '.');
                        try {
                            return Class.forName(s2);
                        }
                        catch(ClassNotFoundException unused_ex) {
                        }
                    }
                    break;
                }
                case 83: {
                    return Short.TYPE;
                }
                case 86: {
                    return Void.TYPE;
                }
                case 90: {
                    return Boolean.TYPE;
                }
                case 91: {
                    return Array.newInstance(ReflectionHelper.a(s, arr_v), 0).getClass();
                }
                default: {
                    f.Log(5, "! parseType; " + ((char)v1) + " is not known!");
                    return null;
                }
            }
        }
        return null;
    }

    private static void a(a reflectionHelper$a0, Member member0) {
        synchronized(ReflectionHelper.class) {
            reflectionHelper$a0.a = member0;
            ReflectionHelper.a[reflectionHelper$a0.hashCode() & ReflectionHelper.a.length - 1] = reflectionHelper$a0;
        }
    }

    private static boolean a(a reflectionHelper$a0) {
        synchronized(ReflectionHelper.class) {
            a reflectionHelper$a1 = ReflectionHelper.a[reflectionHelper$a0.hashCode() & ReflectionHelper.a.length - 1];
            if(!reflectionHelper$a0.equals(reflectionHelper$a1)) {
                return false;
            }
            reflectionHelper$a0.a = reflectionHelper$a1.a;
            return true;
        }
    }

    private static Class[] a(String s) {
        int[] arr_v = {0};
        int v = 0;
        ArrayList arrayList0 = new ArrayList();
        while(arr_v[0] < s.length()) {
            Class class0 = ReflectionHelper.a(s, arr_v);
            if(class0 == null) {
                break;
            }
            arrayList0.add(class0);
        }
        Class[] arr_class = new Class[arrayList0.size()];
        for(Object object0: arrayList0) {
            arr_class[v] = (Class)object0;
            ++v;
        }
        return arr_class;
    }

    protected static boolean beginProxyCall(long v) {
        synchronized(ReflectionHelper.class) {
            if(v == ReflectionHelper.b) {
                ++ReflectionHelper.c;
                return true;
            }
        }
        return false;
    }

    protected static void endProxyCall() {
        synchronized(ReflectionHelper.class) {
            long v1 = ReflectionHelper.c - 1L;
            ReflectionHelper.c = v1;
            if(0L == v1 && ReflectionHelper.d) {
                ReflectionHelper.class.notifyAll();
            }
        }
    }

    protected static void endUnityLaunch() {
        synchronized(ReflectionHelper.class) {
            try {
                ++ReflectionHelper.b;
                ReflectionHelper.d = true;
            label_5:
                while(ReflectionHelper.c > 0L) {
                    ReflectionHelper.class.wait();
                }
            }
            catch(InterruptedException unused_ex) {
                f.Log(6, "Interrupted while waiting for all proxies to exit.");
                if(true) {
                    ReflectionHelper.d = false;
                    return;
                }
                goto label_5;
            }
        }
        ReflectionHelper.d = false;
    }

    protected static Constructor getConstructorID(Class class0, String s) {
        Constructor constructor0;
        a reflectionHelper$a0 = new a(class0, "", s);
        if(ReflectionHelper.a(reflectionHelper$a0)) {
            constructor0 = (Constructor)reflectionHelper$a0.a;
        }
        else {
            Class[] arr_class = ReflectionHelper.a(s);
            Constructor[] arr_constructor = class0.getConstructors();
            Constructor constructor1 = null;
            float f = 0.0f;
            for(int v = 0; v < arr_constructor.length; ++v) {
                Constructor constructor2 = arr_constructor[v];
                float f1 = ReflectionHelper.a(Void.TYPE, constructor2.getParameterTypes(), arr_class);
                if(f1 > f) {
                    if(f1 != 1.0f) {
                        constructor1 = constructor2;
                        f = f1;
                        continue;
                    }
                    constructor1 = constructor2;
                    break;
                }
            }
            ReflectionHelper.a(reflectionHelper$a0, constructor1);
            constructor0 = constructor1;
        }
        if(constructor0 == null) {
            throw new NoSuchMethodError("<init>" + s + " in class " + class0.getName());
        }
        return constructor0;
    }

    protected static Field getFieldID(Class class0, String s, String s1, boolean z) {
        Field field0;
        a reflectionHelper$a0 = new a(class0, s, s1);
        if(ReflectionHelper.a(reflectionHelper$a0)) {
            field0 = (Field)reflectionHelper$a0.a;
        }
        else {
            Class[] arr_class = ReflectionHelper.a(s1);
            float f = 0.0f;
            Field field1 = null;
            while(class0 != null) {
                Field[] arr_field = class0.getDeclaredFields();
                for(int v = 0; v < arr_field.length; ++v) {
                    Field field2 = arr_field[v];
                    if(z == Modifier.isStatic(field2.getModifiers()) && field2.getName().compareTo(s) == 0) {
                        float f1 = ReflectionHelper.a(field2.getType(), null, arr_class);
                        if(f1 > f) {
                            field1 = field2;
                            if(Float.compare(f1, 1.0f) == 0) {
                                f = f1;
                                break;
                            }
                            else {
                                f = f1;
                            }
                        }
                    }
                }
                if(f == 1.0f || class0.isPrimitive() || class0.isInterface() || class0.equals(Object.class) || class0.equals(Void.TYPE)) {
                    ReflectionHelper.a(reflectionHelper$a0, field1);
                    field0 = field1;
                }
                else {
                    class0 = class0.getSuperclass();
                    continue;
                }
                goto label_31;
            }
            ReflectionHelper.a(reflectionHelper$a0, field1);
            field0 = field1;
        }
    label_31:
        if(field0 == null) {
            throw new NoSuchFieldError(String.format("no %s field with name=\'%s\' signature=\'%s\' in class L%s;", (z ? "static" : "non-static"), s, s1, class0.getName()));
        }
        return field0;
    }

    protected static String getFieldSignature(Field field0) {
        Class class0 = field0.getType();
        if(class0.isPrimitive()) {
            String s = class0.getName();
            if("boolean".equals(s)) {
                return "Z";
            }
            if("byte".equals(s)) {
                return "B";
            }
            if("char".equals(s)) {
                return "C";
            }
            if("double".equals(s)) {
                return "D";
            }
            if("float".equals(s)) {
                return "F";
            }
            if("int".equals(s)) {
                return "I";
            }
            if("long".equals(s)) {
                return "J";
            }
            return "short".equals(s) ? "S" : s;
        }
        return class0.isArray() ? class0.getName().replace('.', '/') : "L" + class0.getName().replace('.', '/') + ";";
    }

    protected static Method getMethodID(Class class0, String s, String s1, boolean z) {
        Method method0;
        a reflectionHelper$a0 = new a(class0, s, s1);
        if(ReflectionHelper.a(reflectionHelper$a0)) {
            method0 = (Method)reflectionHelper$a0.a;
        }
        else {
            Class[] arr_class = ReflectionHelper.a(s1);
            Method method1 = null;
            float f = 0.0f;
            while(class0 != null) {
                Method[] arr_method = class0.getDeclaredMethods();
                for(int v = 0; v < arr_method.length; ++v) {
                    Method method2 = arr_method[v];
                    if(z == Modifier.isStatic(method2.getModifiers()) && method2.getName().compareTo(s) == 0) {
                        float f1 = ReflectionHelper.a(method2.getReturnType(), method2.getParameterTypes(), arr_class);
                        if(f1 > f) {
                            if(f1 == 1.0f) {
                                method1 = method2;
                                f = 1.0f;
                                break;
                            }
                            else {
                                method1 = method2;
                                f = f1;
                            }
                        }
                    }
                }
                if(f == 1.0f || class0.isPrimitive() || class0.isInterface() || class0.equals(Object.class) || class0.equals(Void.TYPE)) {
                    ReflectionHelper.a(reflectionHelper$a0, method1);
                    method0 = method1;
                }
                else {
                    class0 = class0.getSuperclass();
                    continue;
                }
                goto label_32;
            }
            ReflectionHelper.a(reflectionHelper$a0, method1);
            method0 = method1;
        }
    label_32:
        if(method0 == null) {
            throw new NoSuchMethodError(String.format("no %s method with name=\'%s\' signature=\'%s\' in class L%s;", (z ? "static" : "non-static"), s, s1, class0.getName()));
        }
        return method0;
    }

    private static native void nativeProxyFinalize(long arg0) {
    }

    private static native Object nativeProxyInvoke(long arg0, String arg1, Object[] arg2) {
    }

    private static native void nativeProxyLogJNIInvokeException(long arg0) {
    }

    protected static Object newProxyInstance(UnityPlayer unityPlayer0, long v, Class class0) {
        return ReflectionHelper.newProxyInstance(unityPlayer0, v, new Class[]{class0});
    }

    protected static Object newProxyInstance(UnityPlayer unityPlayer0, long v, Class[] arr_class) {
        com.unity3d.player.ReflectionHelper.1 reflectionHelper$10 = new c() {
            private Runnable d;
            private UnityPlayer e;
            private long f;
            private long g;
            private boolean h;

            {
                long v = v;  // 捕获的参数 （可能与外部方法变量命名冲突；考虑手动重命名）
                UnityPlayer unityPlayer0 = unityPlayer0;  // 捕获的参数 （可能与外部方法变量命名冲突；考虑手动重命名）
                Class[] arr_class = arr_class;  // 捕获的参数 （可能与外部方法变量命名冲突；考虑手动重命名）
                this.d = new b(0L, v);
                this.e = unityPlayer0;
                this.f = 0L;
            }

            private Object a(Object object0, Method method0, Object[] arr_object) {
                try {
                    if(arr_object == null) {
                        arr_object = new Object[0];
                    }
                    Class class0 = method0.getDeclaringClass();
                    Constructor constructor0 = MethodHandles.Lookup.class.getDeclaredConstructor(Class.class, Integer.TYPE);
                    constructor0.setAccessible(true);
                    return ((MethodHandles.Lookup)constructor0.newInstance(class0, 2)).in(class0).unreflectSpecial(method0, class0).bindTo(object0).invokeWithArguments(arr_object);
                }
                catch(NoClassDefFoundError unused_ex) {
                    f.Log(6, "Java interface default methods are only supported since Android Oreo");
                    ReflectionHelper.nativeProxyLogJNIInvokeException(this.g);
                    return null;
                }
            }

            @Override  // com.unity3d.player.ReflectionHelper$c
            public final void a(long v, boolean z) {
                this.g = v;
                this.h = z;
            }

            @Override
            protected final void finalize() {
                this.e.queueGLThreadEvent(this.d);
                super.finalize();
            }

            @Override
            public final Object invoke(Object object0, Method method0, Object[] arr_object) {
                if(!ReflectionHelper.beginProxyCall(this.f)) {
                    f.Log(6, "Scripting proxy object was destroyed, because Unity player was unloaded.");
                    return null;
                }
                try {
                    this.g = 0L;
                    this.h = false;
                    String s = method0.getName();
                    Object object1 = ReflectionHelper.nativeProxyInvoke(v, s, arr_object);
                    if(this.h) {
                        if((method0.getModifiers() & 0x400) == 0) {
                            return this.a(object0, method0, arr_object);
                        }
                        ReflectionHelper.nativeProxyLogJNIInvokeException(this.g);
                    }
                    else {
                        long v1 = this.g;
                        if(v1 != 0L) {
                            ReflectionHelper.nativeProxyLogJNIInvokeException(v1);
                        }
                    }
                    return object1;
                }
                finally {
                    ReflectionHelper.endProxyCall();
                }
            }
        };
        return Proxy.newProxyInstance(ReflectionHelper.class.getClassLoader(), arr_class, reflectionHelper$10);
    }

    protected static void setNativeExceptionOnProxy(Object object0, long v, boolean z) {
        ((c)Proxy.getInvocationHandler(object0)).a(v, z);
    }
}

