package androidx.compose.ui.autofill;

import android.view.ViewStructure;
import android.view.autofill.AutofillId;
import android.view.autofill.AutofillValue;
import kotlin.Metadata;
import kotlin.jvm.internal.Intrinsics;

@Metadata(d1 = {"\u0000F\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000B\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0004\n\u0002\u0010\u0002\n\u0000\n\u0002\u0010\u0011\n\u0002\u0010\u000E\n\u0002\b\u0004\n\u0002\u0010\b\n\u0002\b\u0003\n\u0002\u0010\r\n\u0000\bÁ\u0002\u0018\u00002\u00020\u0001B\u0007\b\u0002¢\u0006\u0002\u0010\u0002J\u0012\u0010\u0003\u001A\u0004\u0018\u00010\u00042\u0006\u0010\u0005\u001A\u00020\u0006H\u0007J\u0010\u0010\u0007\u001A\u00020\b2\u0006\u0010\t\u001A\u00020\nH\u0007J\u0010\u0010\u000B\u001A\u00020\b2\u0006\u0010\t\u001A\u00020\nH\u0007J\u0010\u0010\f\u001A\u00020\b2\u0006\u0010\t\u001A\u00020\nH\u0007J\u0010\u0010\r\u001A\u00020\b2\u0006\u0010\t\u001A\u00020\nH\u0007J#\u0010\u000E\u001A\u00020\u000F2\u0006\u0010\u0005\u001A\u00020\u00062\f\u0010\u0010\u001A\b\u0012\u0004\u0012\u00020\u00120\u0011H\u0007¢\u0006\u0002\u0010\u0013J \u0010\u0014\u001A\u00020\u000F2\u0006\u0010\u0005\u001A\u00020\u00062\u0006\u0010\u0015\u001A\u00020\u00042\u0006\u0010\u0016\u001A\u00020\u0017H\u0007J\u0018\u0010\u0018\u001A\u00020\u000F2\u0006\u0010\u0005\u001A\u00020\u00062\u0006\u0010\u0019\u001A\u00020\u0017H\u0007J\u0010\u0010\u001A\u001A\u00020\u001B2\u0006\u0010\t\u001A\u00020\nH\u0007¨\u0006\u001C"}, d2 = {"Landroidx/compose/ui/autofill/AutofillApi26Helper;", "", "()V", "getAutofillId", "Landroid/view/autofill/AutofillId;", "structure", "Landroid/view/ViewStructure;", "isDate", "", "value", "Landroid/view/autofill/AutofillValue;", "isList", "isText", "isToggle", "setAutofillHints", "", "hints", "", "", "(Landroid/view/ViewStructure;[Ljava/lang/String;)V", "setAutofillId", "parent", "virtualId", "", "setAutofillType", "type", "textValue", "", "ui_release"}, k = 1, mv = {1, 5, 1}, xi = 0x30)
public final class AutofillApi26Helper {
    public static final AutofillApi26Helper INSTANCE;

    static {
        AutofillApi26Helper.INSTANCE = new AutofillApi26Helper();
    }

    public final AutofillId getAutofillId(ViewStructure viewStructure0) {
        Intrinsics.checkNotNullParameter(viewStructure0, "structure");
        return viewStructure0.getAutofillId();
    }

    public final boolean isDate(AutofillValue autofillValue0) {
        Intrinsics.checkNotNullParameter(autofillValue0, "value");
        return autofillValue0.isDate();
    }

    public final boolean isList(AutofillValue autofillValue0) {
        Intrinsics.checkNotNullParameter(autofillValue0, "value");
        return autofillValue0.isList();
    }

    public final boolean isText(AutofillValue autofillValue0) {
        Intrinsics.checkNotNullParameter(autofillValue0, "value");
        return autofillValue0.isText();
    }

    public final boolean isToggle(AutofillValue autofillValue0) {
        Intrinsics.checkNotNullParameter(autofillValue0, "value");
        return autofillValue0.isToggle();
    }

    public final void setAutofillHints(ViewStructure viewStructure0, String[] arr_s) {
        Intrinsics.checkNotNullParameter(viewStructure0, "structure");
        Intrinsics.checkNotNullParameter(arr_s, "hints");
        viewStructure0.setAutofillHints(arr_s);
    }

    public final void setAutofillId(ViewStructure viewStructure0, AutofillId autofillId0, int v) {
        Intrinsics.checkNotNullParameter(viewStructure0, "structure");
        Intrinsics.checkNotNullParameter(autofillId0, "parent");
        viewStructure0.setAutofillId(autofillId0, v);
    }

    public final void setAutofillType(ViewStructure viewStructure0, int v) {
        Intrinsics.checkNotNullParameter(viewStructure0, "structure");
        viewStructure0.setAutofillType(v);
    }

    public final CharSequence textValue(AutofillValue autofillValue0) {
        Intrinsics.checkNotNullParameter(autofillValue0, "value");
        CharSequence charSequence0 = autofillValue0.getTextValue();
        Intrinsics.checkNotNullExpressionValue(charSequence0, "value.textValue");
        return charSequence0;
    }
}

